...
10. Store only the encoded immutable capability string on the session until the asynchronous launch request consumes it. Ensure collection occurs on the GUI/display thread, performs one bounded profile read per launch, records elapsed collection time with a focused latency test, and does not repeat profile scans.
...
5. Add a final GUI-thread validation immediately before `Session::start()` creates `AsyncConnectionStartThread`: retain or recreate the hidden test window, re-resolve the selected screen and complete identity tuple, and recollect or clear the immutable capability snapshot if anything changed. Add request serialization tests for both verbs, HDR/non-HDR, GFE/non-GFE, empty payload, and percent-encoding.
...
3. Compatibility:
   ...
   - before claiming the broader non-GFE sender scope, exercise representative supported Sunshine/Apollo/Vibepollo-family baselines using both launch and resume. If any claimed family rejects unknown keys, freeze an explicit server-family/version gate from the available `NvComputer` server-info fields and omit the capability before request construction; do not leave this as an implementation-time guess;
   ...
   - all `set_runtime_config_overrides`/clear paths, including WebRTC, live RTX HDR editing, and process termination, preserve map-plus-owner consistency.
