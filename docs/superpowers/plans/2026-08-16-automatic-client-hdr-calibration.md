...
5. Add the ephemeral `NvComputer::clientDisplayCapabilitiesVersion` field with a zero initializer. Parse `/serverinfo` into a local zero value on every refresh, accept only integer `1`, assign it through the existing refresh/merge `ASSIGN_IF_CHANGED` list, and reset it on missing, malformed, zero, unknown, or changed-host responses. Do not add it to settings persistence or serialized equality. Add refresh fixtures for version 1 followed by absent/zero/malformed/2 and assert no stale advertisement survives.
6. Define a concrete GUI-owned `client_display_preparation_t` handoff. `Session` owns a GUI-thread-only RAII hidden probe-window member and an optional preparation member with states `unprepared`, `prepared`, `invalidated`, and `handed-off`. Initialization failure destroys the window and invalidates the preparation; cancellation or repeated `start()` cannot hand off stale state. `Session::start()` is GUI-thread-only and, immediately before creating `AsyncConnectionStartThread`, invokes `finalize_client_display_preparation()`, reuses or recreates the probe window, re-resolves the selected QScreen/SDL/DISPLAYCONFIG/DXGI identity, and recollects or clears the value if any identity/descriptor differs. It then atomically moves only a copied immutable value snapshot (including the encoded string and selected-display identity) into the worker constructor, destroys the probe window on the GUI thread, and marks the preparation handed-off. The worker may call `NvHTTP::startApp()` with that string but may not call QScreen, SDL, DISPLAYCONFIG, DXGI, or ColorProfile APIs. Add request serialization tests for both verbs, HDR/non-HDR, advertised/unadvertised/GFE, empty payload, and percent-encoding, plus thread-affinity tests for initialization, finalization, cancellation, repeated finalization, and screen mutation.
